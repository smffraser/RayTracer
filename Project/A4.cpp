...
double get_fresnel_R(double n1, double n2, glm::vec3 direction, glm::vec3 normal, double cosI, double cosT){
    ...
    // R = 1/2(R_s + R_p)
    // R_s = reflectance of the s-polarized light (perpendicular)
    // R_p = reflectance of the p-polarized light (parallel)
    
    // Note that cosI = cos of the initial angle (see Snell's law) which is
    // the same as the dot product of OG ray direction and the normal
    
    // cosT = cos of the transmitted angle which is the same as the square root of the
    // "under_rooted" calculation done to find the refracted ray
    
    // R_s = ((n_1 * cosI - n_2 * cosT)/(n_1 * cosI + n_2 *cosT))^2
    // R_p = ((n_1 * cosT - n_2 * cosI)/(n_1 * cosT + n_2 *cosI))^2
    
    double R_s = pow(((n1 * cosI - n2 * cosT) / (n1 * cosI + n2 * cosT)), 2);
    double R_p = pow(((n2 * cosI - n1 * cosT) / (n2 * cosI + n1 * cosT)), 2);
    double R = 0.5 * (R_s + R_p);
    
    if (R > 1.0){
        std::cout << "ERROR: fresnel too big!: " << R <<std::endl;
        exit(1);
    } else {
        return R;
    }
...
Ray get_refracted_ray (Ray incident, Intersection intersection, double index, bool& totalInternalReflection, double& fresnel, double& final_ni, double& final_nt) {
    ...
    if (vndot <= 0) {
        ...
        // We need to flip the dot product
        vndot = -vndot;
    } else {
    ...
    double under_root = 1 - (pow(n, 2) * (1-pow(vndot,2)));
    if (under_root < 0) {
        ...
        t_direction = ((-1)*n*vndot - sqrt(under_root))*normal + (n*norm_v);
        fresnel = get_fresnel_R(n_i, n_t, norm_v, normal, vndot, sqrt(under_root));
        final_ni = n_i ;
        final_nt = n_t ;
        return Ray{t_origin, t_direction};
    }
...
glm::vec3 ray_colour(Ray r, glm::vec3 bg, SceneNode *root, const glm::vec3 & ambient, const std::list<Light *> & lights, int reflect_count, int refract_count, int glossy_rays, int soft_rays){
    ...
    if (find_intersection(r, root, intersection)){
        ...
        // in the final colour
        return colour_vec + material->get_ks() * ((fresnel_R * reflected_colour) + ((1.0f - fresnel_R) * refracted_colour));
    }
